...
// - Speed table
// - CV: 1, 2, 3, 4, 5, 7, 8, 17, 18, 27, 28, 29, 33 to 41, 67 to 94.
...
CVPair FactoryDefaultCVs [] =        // Default CV Values Table
{
  ...
  //----------------------------------------------- Function F4 ----------------------------------------------
  {132, 1},     //  0 = LED On/Off, 1 = LED Blink, 2 = Servo, 3 = Audio MP3 (Long,loop), 4 = Audio MP3 (Short), 5 = Shunting/Manoeuvre
  {133, 3},    //  Blink = Rate, Servo = Rate, Audio = Volume(0-30)
  {134, 3},     //  Servo = Start Position, Audio = Audio Track
  {135, 200},   //  Servo = End Position
  //----------------------------------------------- Function F5 ----------------------------------------------
...
void SetPinOutput() {
  ...
  pinMode(0, INPUT_PULLUP);                         // RX Trigger1
  pinMode(8, INPUT_PULLUP);                         // RX Trigger2
}//-------------------------------------- SetEeprom ---------------------------------------------------------------
void SetEeprom() {
...
# if defined BrakeABC
  ...
  if (Step > 17) {
    Step = 0;
    //                                                Calcul ABC après 16 échantillons
    ...
    if (Commande == '7')                           // Trace ABC sur Traceur série
    {
      ...
      Serial.println(MoyenneTrack);                // Ligne verte, différence de tension
        ...
        if (MoyenneTrack > cv26) {                       // (15 Volts / 255 ) * 10 = 0,6 V.  OK.
          //digitalWrite(LED_PIN_FL, 1);
          switch (cv27) {
...
void setup() {
  ...
  wdt_enable(WDTO_500MS);                             // Init Watchdog 120 ms.
}
//---------------------------------------------- Loop (2.2 ms.) -----------------------------------------------------------------------------
void loop() {
  wdt_reset();                                       // Watchdog
